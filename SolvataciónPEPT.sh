...
#  SOLV.sh
#  SCRIPT PARA EVALUAR LA ESTABILIDAD DE LOS PÃ‰PTIDOS CATIÃ“NICOS ANTIMICROBIANOS EN AGUA
...
# CREANDO CARPETAS DE SIMULACIÃ“N
mkdir ~/SOLV
mkdir ~/SOLV/$pept
cp ~/PDB/$pept.pdb ~/SOLV/$pept/
cd ~/SOLV/$pept/

# GENERA EL ARCHIVO DE ADICIÃ“N DE IONES
cat > ions.mdp << EOF
integrator      = steep
emtol		= 1000.0
emstep          = 0.01
...
# GENERA EL ARCHIVO DE MINIMIZACIÃ“N DE ENERGÃA
cat > minim.mdp << EOF
integrator	= steep
emtol		= 1000.0
emstep      	= 0.01
...
# DEFINIR EL TAMAÃ‘O DE LA CAJA DE SIMULACIÃ“N
...
13 # AÃ±ade los iones necesarios al solvente para estabilizar el sistema
EOF

# MINIMIZACIÃ“N DE ENERGÃA
...
grompp_mpi -f minim.mdp -c $pept-ION.gro -p topol.top -o em-$pept.tpr
mpirun -np 8 mdrun_mpi -v -deffnm em-$pept
fi
